...
// Odds text color
const ODDS_COLORS = (odds) =>
  odds <= 1.5 ? "text-green-600" : "text-red-600";
...
function getCountdown(matchTime, status, now) {
  ...
  const normalized = matchTime.includes("T")
    ? matchTime
    : matchTime.replace(" ", "T");
    ...
    >
      ...
      <div className="flex justify-between items-center bg-gray-50 rounded p-2 mb-2">
        <div className="flex-1 text-sm">
          ...
          <div
            className={`text-xs ${ODDS_COLORS(match.odds)}`}
          >
            Odds:{" "}
            {match.odds
...
export default function Dashboard() {
  ...
  const [scrollMaxHeight, setScrollMaxHeight] =
    useState("300px");
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      const filtered = (
        ...
        const { utc, local } =
          getMatchDateStrings(row.match_time);
      ...
      setStats({
        ...
        HOME: {
          rate:
            res.data.match_outcome_win_rate?.HOME || 0,
          count:
            res.data.match_outcome_counts?.HOME || 0,
        },

        AWAY: {
          rate:
            res.data.match_outcome_win_rate?.AWAY || 0,
          count:
            res.data.match_outcome_counts?.AWAY || 0,
        },

        DRAW: {
          rate:
            res.data.match_outcome_win_rate?.DRAW || 0,
          count:
            res.data.match_outcome_counts?.DRAW || 0,
        },

        YES: {
          rate: res.data.yes_win_rate || 0,
          count: res.data.yes_count || 0,
        },

        OVER: {
          rate: res.data.over_win_rate || 0,
          count: res.data.over_count || 0,
        },

        GENERAL: {
          rate: res.data.general_win_rate || 0,
          count: res.data.general_count || 0,
        },

        liveGames: filtered.filter(
          (m) =>
            ![
              "TIMED",
              "FINISHED",
  ...
  useLayoutEffect(() => {
    function recalcHeight() {
      ...
      // visualViewport gives a more accurate viewport
      // on mobile Chrome and Firefox.
  ...
  return (
    <div className="w-full min-h-0 flex flex-col bg-transparent p-4 md:p-6">
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3 mb-4">
          {[
            ...
            {
              label: "AWAY %",
              value: stats.AWAY.rate,
            },
            {
              label: "DRAW %",
              value: stats.DRAW.rate,
            },
            {
              label: "YES %",
              value: stats.YES.rate,
            },
            {
              label: "OVER %",
              value: stats.OVER.rate,
            },
            {
              label: "General %",
              value: stats.GENERAL.rate,
            },
            {
              label: "Live",
            ...
            <Card
              key={i}
              className={`rounded-xl shadow-sm ${STATS_COLORS[i]}`}
            >
        ...
        <div className="mb-4 p-4 bg-purple-50 rounded-lg text-gray-700">
          ...
          <p>
            Welcome to Lilymac Predictions Hub,
            your trusted platform for accurate and
            smart betting insights. We provide daily
            top predictions, live updates, and VIP
            tips to help you make informed decisions
            and enjoy smarter wins every day. Join
            our community and elevate your betting
      ...
      <div className="flex-1 min-h-0 relative">
        ...
        <Card className="h-full min-h-0 rounded-2xl shadow-lg border border-transparent flex flex-col p-4 relative">

          <h3
            ref={titleRef}
            className="text-lg font-bold mb-3 text-red-600"
          >
            🔥 Top Predictions
          </h3>
